...
test_json_endpoint "GET" "$BASE_URL/straatnamen/https%3A%2F%2Fn2t.net%2Fark%3A%2F60537%2Fbn4b1Q" 200 "Geef informatie over de Lombardsteeg (https://n2t.net/ark:/60537/bn4b1Q); valideer geometry + alt_names + eerste_vermelding; zonder problematisch geen newline in genoemd_naar" "application/json" '.identifier and .naam and .type and (.geometry.type | IN("LineString","MultiLineString","Polygon")) and (.alt_names | type == "array") and .eerste_vermelding == "1504" and .problematisch == null and (.genoemd_naar | contains("\n") | not)'
test_json_endpoint "GET" "$BASE_URL/straatnamen/https%3A%2F%2Fn2t.net%2Fark%3A%2F60537%2Fb4N7Fm" 200 "Geef informatie over het Houtmanspad (https://n2t.net/ark:/60537/b4N7Fm); valideer dat problematisch-uitleg na een newline in genoemd_naar zit en eerste_vermelding null is" "application/json" '.identifier and .naam and (.problematisch | type == "string") and (.problematisch | length > 0) and .genoemd_naar == ("Genoemd naar zie Houtmansgracht.\n" + .problematisch) and .eerste_vermelding == null'
