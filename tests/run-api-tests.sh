...
test_json_endpoint "GET" "$BASE_URL/straatnamen/https%3A%2F%2Fn2t.net%2Fark%3A%2F60537%2Fbn4b1Q" 200 "Geef informatie over de Lombardsteeg (https://n2t.net/ark:/60537/bn4b1Q); valideer geometry + alt_names + sinds; zonder problematisch geen newline in genoemd_naar" "application/json" '.identifier and .naam and .type and (.geometry.type | IN("LineString","MultiLineString","Polygon")) and (.alt_names | type == "array") and .sinds == 1504 and .problematisch == null and (.genoemd_naar | contains("\n") | not)'
test_json_endpoint "GET" "$BASE_URL/straatnamen/https%3A%2F%2Fn2t.net%2Fark%3A%2F60537%2Fb4N7Fm" 200 "Geef informatie over het Houtmanspad (https://n2t.net/ark:/60537/b4N7Fm); valideer dat problematisch-uitleg na een newline achteraan genoemd_naar zit" "application/json" '.identifier and .naam and (.problematisch | type == "string") and (.problematisch | length > 0) and (.problematisch as $p | (.genoemd_naar | endswith("\n" + $p)) and (.genoemd_naar | length > ($p | length) + 1))'
test_json_endpoint "GET" "$BASE_URL/straatnamen/https%3A%2F%2Fn2t.net%2Fark%3A%2F60537%2Fb9MVd1" 200 "Geef informatie over het Scheygrondplantsoen (https://n2t.net/ark:/60537/b9MVd1); valideer genoemd_naar in vier talen en dat de ontbrekende ligging in alle talen null is" "application/json" '(.genoemd_naar | startswith("Genoemd naar")) and (.genoemd_naar_en | startswith("Named after")) and (.genoemd_naar_fr | startswith("Cette rue")) and (.genoemd_naar_de | startswith("Benannt nach")) and ([.ligging, .ligging_en, .ligging_fr, .ligging_de] | all(. == null)) and (has("ligging_en") and has("ligging_fr") and has("ligging_de"))'
test_json_endpoint "GET" "$BASE_URL/straatnamen/https%3A%2F%2Fn2t.net%2Fark%3A%2F60537%2Fb05w8st" 200 "Geef informatie over de Aletta Jacobskade (https://n2t.net/ark:/60537/b05w8st); valideer dat alleen de Nederlandse genoemd_naar en ligging gevuld zijn en de vertalingen null" "application/json" '(.genoemd_naar | type == "string") and (.ligging | type == "string") and ([.genoemd_naar_en, .genoemd_naar_fr, .genoemd_naar_de, .ligging_en, .ligging_fr, .ligging_de] | all(. == null)) and has("genoemd_naar_en") and has("ligging_de")'
